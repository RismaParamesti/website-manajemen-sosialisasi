...
const Apartement = () => {
  ...
  useEffect(() => {
    // Fetch mall data
    axios
      .get("http://localhost:5000/apartements")
      .then((response) => {
        setData(response.data);
        setHealthFacilities(response.data);
      })
      .catch((error) => {
        console.error("Error fetching Apartement data:", error);
      });
  }, []);
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Apartement");
    XLSX.writeFile(workbook, "Apartement.xlsx");
  };

  const handleExportPDF = () => {
    const doc = new jsPDF();
    doc.text("Data Apartement", 10, 10);
    ...
    <div className="min-h-screen bg-base-200 px-6 py-10 space-y-12">
      ...
      <div className="bg-base-100 p-6 rounded-xl shadow-lg">
        <h3 className="text-center font-semibold text-xl mb-1">
          Persentase Sosialisasi Apartement Berdasarkan Wilayah
        </h3>
